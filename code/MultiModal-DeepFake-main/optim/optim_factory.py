...
def add_weight_lr(args, model, weight_decay=1e-5, skip_list=()):
    decay = []
    no_decay = []
    ...
    return [{'params': lr_vis, 'lr': args.lr},
        {'params': lr_text, 'lr': args.lr_text}]
    
def add_weight_lr_img(args, model, weight_decay=1e-5, skip_list=()):
    decay = []
    no_decay = []
    ...
    return [{'params': lr_vis, 'lr': args.lr_img},
        {'params': lr_text, 'lr': args.lr}]
...
def create_optimizer(args, model, filter_bias_and_bn=True):
    ...
    if weight_decay and filter_bias_and_bn:
        ...
        if 'lr_text' in args: 
            parameters = add_weight_lr(args, model, weight_decay, skip)
        if 'lr_img' in args: 
            parameters = add_weight_lr_img(args, model, weight_decay, skip)
        else:
