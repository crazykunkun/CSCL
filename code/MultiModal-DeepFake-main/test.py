...
from utils import visualize_res
...
def evaluation(args, model, data_loader, tokenizer, device, config):
    ...
    for i, (image, label, text, fake_image_box, fake_word_pos, W, H, image_path) in enumerate(metric_logger.log_every(args, data_loader, print_freq, header)):
        ...
        #     label_cls = label[ii]
        #     # if label_cls=='text_swap' or label_cls=='text_attribute':
        #     if label_cls=='face_swap' or label_cls=='face_attribute':
        #         sub_mask.append(False)
        #     else:
        #         sub_mask.append(True)
        #         label_new.append(label_cls)
        #         text_new.append(text[ii])
        
        # sub_mask = torch.from_numpy(np.array(sub_mask))
        # image = image[sub_mask]
        # fake_image_box = fake_image_box[sub_mask]
        # fake_word_pos = fake_word_pos[sub_mask]
        # label = label_new
        # text = text_new
        ...
        # visualize_res({'img':image_path, 'label':label, 'text':text, 'gt_box':fake_image_box, 'gt_text':fake_word_pos},
        #               {'binary':logits_real_fake, 'multi':logits_multicls, 'box':output_coord, 'text':logits_tok})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_epoch', default='49', type=str)
